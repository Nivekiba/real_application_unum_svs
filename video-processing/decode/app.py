...
def load_video(benchmark_bucket, bucket, blob, dest_dir):
    ...
    # Download from S3: Bucket=benchmark_bucket, Key=f"{bucket}/{blob}"
    # s3.download_file(benchmark_bucket, f"{bucket}/{blob}", path)
    s3.download_file(bucket, blob, path)
    return path
...
def upload_imgs(benchmark_bucket, bucket, paths):
    for path in paths:
        ...
        key = f"{bucket}/{name}"
        s3.upload_file(path, benchmark_bucket, name)
        # s3.upload_file(name, benchmark_bucket, key)
        # yield key
        yield name
