...
def download_bin(benchmark_bucket, bucket, name, dest_dir):
    ...
    if not os.path.exists(path):
        # s3.download_file(benchmark_bucket, bucket + '/' + name, path)
        s3.download_file(benchmark_bucket, name, path)
...
def upload_files(benchmark_bucket, bucket, paths, prefix):
    for path in paths:
        ...
        file = prefix + file
        # s3.upload_file(path, benchmark_bucket, bucket + '/' + file)
        s3.upload_file(path, benchmark_bucket, file)
...
def lambda_handler(event, context):
    ...
    for path in input_paths:
        ...
        if ".y4m" in file:
            #s3.download_file(benchmark_bucket, input_bucket + '/' + file, path)
            s3.download_file(benchmark_bucket, "vid/" + file, path)
        else:
            file = prefix + file
            #s3.download_file(benchmark_bucket, output_bucket + '/' + file, path)
            s3.download_file(benchmark_bucket, file, path)
